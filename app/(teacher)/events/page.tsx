...
export default async function EventsPage() {
  ...
  const rows = (events ?? []).map((event) => {
    const rsvps = (event.event_rsvps as { status: RsvpStatus }[] | null) ?? [];
    return {
      id: event.id,
      title: event.title,
      ...
      when: formatEventWhen(event.starts_at, event.ends_at, policy.timezone),
      studentCount:
        (event.event_students as { id: string }[] | null)?.length ?? 0,
